...
class Poller:
    ...
    def _start_run(self, issue: dict) -> bool:
        ...
        tracer.pipeline_decision(issue_id, "task_classified",
                                 task_type=task_type,
        ...
        if not current or current["state"]["name"] != self._config.eligibility_state:
            ...
            tracer.pipeline_decision(issue_id, "skipped_not_eligible",
                                     state=current["state"]["name"] if current else "not_found")
            return False

        # Dependency check — skip if any blocking issue is not Done/Cancelled
        blockers = self._check_active_blockers(current)
        if blockers:
            tracer.pipeline_decision(issue_id, "blocked",
                                     blocked_by=[b.get("identifier") for b in blockers])
    ...
    def _handle_result(self, result: RunResult) -> None:
        ...
        tracer.pipeline_decision(
            issue_id, "run_result",
            exit_code=result.exit_code,
            signal_type=(signal or {}).get("type"),
            error=result.error,
        )
    ...
    def _finish_block_done(self, issue_id: str, issue: Optional[dict], result: RunResult) -> None:
        ...
        if worktree_path and self._config.github_token:
            self._ensure_worktree_readme(worktree_path, issue_id, issue, branch, summary)
            try:
    ...
    def _ensure_worktree_readme(
        ...
        """Write README.md into the worktree (first block to finish) and commit it."""
        readme = Path(worktree_path) / "README.md"
        if readme.exists():
            return

        project_name = self._workspace._project_slug or "Project"
        issue_title = (issue or {}).get("title", issue_id)
        ...
        lines = [
            f"# {project_name}",
            "",
            f"This branch (`{branch}`) was created by [Resonance](https://github.com/QueenOne/agentic-pipeline) — a supervised agentic delivery pipeline.",
            "",
            "## Current block",
            "",
            f"**{issue_title}**" + (f"  \n[View in Linear]({issue_url})" if issue_url else ""),
            "",
        ]
        if summary:
            lines += ["## What was implemented", "", summary, ""]
        if github_url:
            lines += [f"[View branch on GitHub]({github_url})", ""]
        lines += [
            "---",
            f"_Auto-generated by Resonance · {now_str}_",
        ]
        ...
        try:
            subprocess.run(["git", "add", "README.md"], cwd=worktree_path, check=True,
                           capture_output=True, env=_res_env)
            subprocess.run(["git", "commit", "-m", f"docs: add README (Resonance bootstrap) [{issue_id}]"],
                           cwd=worktree_path, check=True, capture_output=True, env=_res_env)
            logger.info("committed README.md in worktree issue=%s", issue_id)
        except subprocess.CalledProcessError as e:
            logger.warning("could not commit README.md issue=%s: %s", issue_id, e)
